...
class TestEvaluationResult:
    ...
    def test_score_extracted(self):
        ...
        r = EvaluationResult.from_dict({"score": 7, "is_correct": True, "feedback": "ok"})
        assert r.score == 7

    def test_is_correct_extracted(self):
        """is_correct is read from the data dict."""
        r = EvaluationResult.from_dict({"score": 0, "is_correct": False, "feedback": "bad"})
        assert r.is_correct is False

    def test_feedback_extracted(self):
        """feedback is read from the data dict."""
        r = EvaluationResult.from_dict({"score": 5, "is_correct": True, "feedback": "decent"})
        assert r.feedback == "decent"

    def test_missing_points_default_empty(self):
        """missing_points defaults to [] when absent."""
        r = EvaluationResult.from_dict({"score": 0, "is_correct": False, "feedback": ""})
